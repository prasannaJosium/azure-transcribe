...
# API_KEY = confs.PRIMARY_ACCO_API_KEY
...
SUBSCRIPTION_KEY = confs.ACCOUNT_API_KEY
SERVICE_REGION = confs.SERVICE_REGION

NAME = "Audio Transcriptions"
DESCRIPTION = "Transcribes audio channels of multimedia files"
...
def transcribe(filename, sas_url, audio_source):
    ...
    transcription_definition = transcribe_from_single_blob(sas_url, properties)
    ...
    while not completed:
        ...
        if transcription.status == "Succeeded":
            ...
            for file_data in _paginate(api, pag_files):
                ...
                # jsonfile_name = path.join(confs.transcripts_path, file + '.json')
                json_result = json.loads(results.content.decode('utf-8'))
                json_result['origin_filename'] = audio_source
                with open(filename, 'w') as fs:
                    json.dump(json_result, fs)
                # logging.info(f"Results for {audiofilename}:\n{results.content.decode('utf-8')}")
        elif transcription.status == "Failed":
            logging.info(f"Transcription failed: {transcription.properties.error.message}")
            errorresponse = api.get_transcription_files(transcription_id)
            print(errorresponse)


def transcribe_audio():
    filename = os.path.join(confs.base_loc, 'blob', 'links.txt')
    with open(filename, 'r') as sas_links:
        for link in sas_links:
            filename, sas_url = link.split(',', maxsplit=1)

            file, ext = path.splitext(filename)
            trans_file_name = path.join(confs.transcripts_path, file + '.json')

            if path.exists(trans_file_name):
                logging.debug('Already transcribed {0}'.format(file))
                continue

            transcribe(trans_file_name, sas_url, (file + '.m4'))
